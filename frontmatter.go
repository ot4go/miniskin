...
// parseFrontMatter splits a file into front-matter key-value pairs and body content.
...
func parseFrontMatter(content string) (vars map[string]string, body string, err error) {
	...
	for lineNum, line := range strings.Split(header, "\n") {
		...
		if colon == -1 {
			return nil, "", fmt.Errorf("front-matter line %d: missing colon in %q", lineNum+1, line)
		}
		key := strings.TrimSpace(line[:colon])
		val := strings.TrimSpace(line[colon+1:])
		if key == "" {
			return nil, "", fmt.Errorf("front-matter line %d: empty key", lineNum+1)
		}
		vars[key] = val
	}
